...
struct CalendarEntryAddView: View {
    
    @Environment(\.presentationMode) var presentationMode
    
    @StateObject var viewModel: CalendarEntryAddViewModel
    ...
    private var addButtonView: some View {
        Button(action: viewModel.add) {
            HStack {
                Spacer()
                Text(viewModel.sendingAddRequest ? "" : "Add".localized)
                    .font(.system(size: 20))
                    .fontWeight(.bold)
                    .foregroundColor(.white)
                    .overlay(
                        Group {
                            if viewModel.sendingAddRequest {
                                if viewModel.eventAddedSuccessFully {
            ...
            .frame(width: viewModel.sendingAddRequest ? 60 : UIScreen.main.bounds.width - 30, height: 60)
        }.disabled(!viewModel.validEntry)
        .background(viewModel.validEntry ? Color.HHG_Blue : Color.gray)
        .cornerRadius(viewModel.sendingAddRequest ? 50 : 15)
        .padding(.top, 20)
    }
...
struct CalendarEntryAddView_Previews: PreviewProvider {
    static var previews: some View {
        CalendarEntryAddView(viewModel: CalendarEntryAddViewModel(calendarManager: MonthlyCalendarManager.mock))
    }
}
